...
The organization masthead exposes a national/chapter context and an address-derived chapter theme. Theme palettes use accessible named roles documented in `themes/colorpalette.md`; members can override the suggested Indian Flag, Jaisalmer, Jodhpur, Udaipur, Jaipur, or Peacock palette from the header, and that explicit preference survives reloads until reset to the chapter theme. Navigation is grouped into what is happening and who is here so convention categories stay with the convention rather than becoming global page chrome.

The organization model is represented by `ChapterCore`: IMRC National owns shared programs and the IMRC Convention calendar, while regional chapters are selected from a member's home-city/chapter field and maintain their own committees and initiatives. `#chapter/<id>` presents that hierarchy and a labeled service-area map built from non-overlapping state/province geometry. U.S. geometry comes from the Census Bureau's 2024 cartographic boundary files; simplified Canadian province/territory outlines come from Code for America's public `click_that_hood` dataset. `Voting`/`VotingCore` is a reusable, browser-local poll that national or chapter committees can open at `#votes` when a decision needs member input.

The `#start` landing experience is an interactive Rama demo workflow: Step 01 opens the real sign-in screen with Rama preselected, then a highlighted convention banner and contextual coach marks guide the member through household registration, agenda selection, profile activity, and an example vote. Each step remains directly addressable at `#journey/<step>` with previous/next navigation, while its progress rail is derived from browser-local sign-in, registration, agenda, profile-visit, and participation state. The original welcome survey is preserved as a reusable community-listening poll example on the Votes & polls page.
