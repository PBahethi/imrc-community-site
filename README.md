...
The `#start` landing experience is an interactive Rama demo workflow: each step opens at `#journey/<step>`, links into the real sign-in/convention/profile/poll screen, and provides previous/next navigation. Its progress rail is derived from browser-local sign-in, registration, agenda, profile-visit, and participation state. The original welcome survey is preserved as a reusable community-listening poll example on the Votes & polls page.
