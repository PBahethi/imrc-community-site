(function(root){
  ...
  const agendaSession=(item,day,dayIndex,itemIndex)=>typeof item==='string'?{id:conventionAgendaId(dayIndex,itemIndex),time:'',title:item,category:'General',room:'',dayId:day.id||`day-${dayIndex+1}`,day:day.day}:{...item,dayId:day.id||`day-${dayIndex+1}`,day:day.day};
  ...
  function conventionWorkflow(event,person){
    ...
    const agenda=event.agenda.flatMap((day,dayIndex)=>day.items.map((item,itemIndex)=>agendaSession(item,day,dayIndex,itemIndex)));
    const selectedPacking=plan.packing?.length?plan.packing:packingFor(plan);
    const checkin=globalThis.ConventionExperience?.checkinPanel(event,person,plan.agendaIds||[],data)||'';
    return checkin+`<section class="event-action-panel convention-workflow" aria-labelledby="convention-workflow-title"><div class="event-action-heading"><div><p class="eyebrow">REVIEW WORKFLOW</p><h2 id="convention-workflow-title">Finish convention logistics</h2><p>Register first, book the hotel externally, then return here to finish travel, agenda, reminders, and buses in any order.</p></div><span class="event-capacity">${registered?'Registered':person?'Register above to begin':'Sign in to start'}</span></div>${person?`<div class="workflow-steps"><span class="${registered?'done':''}">1 Register</span><span class="${plan.hotelBooked?'done':''}">2 Hotel</span><span class="${plan.arrivalBus&&plan.departureBus?'done':''}">3 Buses</span><span class="${plan.agendaIds?.length?'done':''}">4 Agenda</span><span class="${plan.reminders?.length?'done':''}">5 Reminders</span></div><form id="convention-workflow-form"><div class="hotel-booking-card"><div><h3>Book hotel externally</h3><p>Use the venue website to reserve rooms or finish an organizer-provided room block booking. Return here and mark the booking complete.</p></div><a class="button secondary" href="${esc(event.venue.sourceUrl)}" target="_blank" rel="noopener">Book hotel externally ↗</a><label class="attendee-choice compact"><input type="checkbox" name="hotelBooked" ${plan.hotelBooked?'checked':''}><span>Hotel booking completed</span></label></div><fieldset><legend>Airline itinerary</legend><div class="logistics-grid"><label>Airline<input name="airline" maxlength="60" value="${esc(plan.airline||'')}" placeholder="American, Southwest, Delta..."></label><label>Arrival flight<input name="flightIn" maxlength="40" value="${esc(plan.flightIn||'')}" placeholder="AA 1234"></label><label>Arrival date/time<input name="arrivalAt" maxlength="40" value="${esc(plan.arrivalAt||'')}" placeholder="2028-01-14 6:20 PM"></label><label>Departure flight<input name="flightOut" maxlength="40" value="${esc(plan.flightOut||'')}" placeholder="WN 5678"></label><label>Departure date/time<input name="departureAt" maxlength="40" value="${esc(plan.departureAt||'')}" placeholder="2028-01-19 10:15 AM"></label></div></fieldset><fieldset><legend>Airport buses</legend><div class="logistics-grid"><label>Airport to hotel<select name="arrivalBus"><option value="">Choose arrival coach</option>${busTimes(event,'arrival').map(t=>`<option value="${esc(t)}" ${plan.arrivalBus===t?'selected':''}>${esc(t)} · OKC to Omni</option>`).join('')}</select></label><label>Hotel to airport<select name="departureBus"><option value="">Choose departure coach</option>${busTimes(event,'departure').map(t=>`<option value="${esc(t)}" ${plan.departureBus===t?'selected':''}>${esc(t)} · Omni to OKC</option>`).join('')}</select></label></div></fieldset><fieldset><legend>Agenda signups</legend><p class="form-note">Choose any sessions you want. Day tabs and categories help you focus; selections across every tab are saved together.</p><div class="agenda-pick-grid">${agenda.map(a=>`<label class="choice agenda-option" data-planner-day="${esc(a.dayId)}" data-planner-category="${esc(a.category)}"><input type="checkbox" name="agendaIds" value="${esc(a.id)}" ${plan.agendaIds?.includes(a.id)?'checked':''}><span><span class="session-category">${esc(a.category)}</span><strong class="session-time">${esc(a.time)} · ${esc(a.title)}</strong><small>${esc(a.day)}</small><span class="session-room">${esc(a.room)}</span></span></label>`).join('')}</div></fieldset><fieldset><legend>Event reminders</legend><div class="choice-grid">${[['hotel','Hotel booking follow-up'],['bus','Bus pickup reminders'],['meals','Meal times'],['agenda','Selected agenda sessions'],['yoga','Yoga mornings'],['packing','Packing list before travel']].map(([id,label])=>`<label class="choice"><input type="checkbox" name="reminders" value="${id}" ${plan.reminders?.includes(id)?'checked':''}><span>${esc(label)}</span></label>`).join('')}</div></fieldset><fieldset><legend>Example packing list</legend><p class="form-note">The demo suggests items from your itinerary and reminders. Adjust the checked items before saving.</p><div class="choice-grid">${packingFor(plan).map(item=>`<label class="choice"><input type="checkbox" name="packing" value="${esc(item)}" ${selectedPacking.includes(item)?'checked':''}><span>${esc(item)}</span></label>`).join('')}</div></fieldset><button class="button" type="submit">Save convention logistics</button><span id="convention-status" role="status"></span></form>`:'<p><a href="#login">Sign in</a> to review the convention workflow.</p>'}</section>`;
  }
  function mount(next,event,refresh=false){
    ...
    bind(event);
    const person=actor(),plan=repo.read().conventionPlans.find(row=>row.eventId===event.id&&row.actorId===person?.id);
    globalThis.ConventionExperience?.decoratePlanner(event);
    globalThis.ConventionExperience?.bindCheckin(event,person,plan?.agendaIds||[],()=>mount(data,event,true),data);
  }
  function bind(event){
